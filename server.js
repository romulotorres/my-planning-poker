...
io.on('connection', (socket) => {
    socket.on('join-room', ({ roomId, userName, isSpectator }) => {
        socket.join(roomId);
        if (!rooms[roomId]) {
            rooms[roomId] = { users: {}, revealed: false, adminId: socket.id, revealedAt: null };
        }
        rooms[roomId].users[socket.id] = { name: userName, vote: null, isSpectator: isSpectator };
    ...
    socket.on('reset-game', (roomId) => {
        if (rooms[roomId] && rooms[roomId].adminId === socket.id) {
            ...
            Object.keys(rooms[roomId].users).forEach(id => rooms[roomId].users[id].vote = null);
        ...
        for (const roomId in rooms) {
            if (rooms[roomId].users[socket.id]) {
                ...
                if (Object.keys(rooms[roomId].users).length === 0) {
                    ...
                    if (wasAdmin) rooms[roomId].adminId = Object.keys(rooms[roomId].users)[0];
                    io.to(roomId).emit('update-room', rooms[roomId]);
                }
            }
        }
